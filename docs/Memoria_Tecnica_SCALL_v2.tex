...
\hypersetup{
    ...
    urlcolor=azulbase,
    citecolor=azulbase,
    pdftitle={Memoria Técnica App-SCALL v2.0},
...
\fancyhead[R]{\small\color{azulbase} Versión 2.0 --- Mayo 2026}
...
\begin{titlepage}
    ...
    {\normalsize Versión 2.0\\
    21 de mayo de 2026\par}
\end{titlepage}
...
\begin{itemize}
    ...
    \item \textbf{Generación de informes:} exportación a PDF (identidad visual Amulén) y Excel con balance diario completo.
\end{itemize}
...
\midrule
1. Datos del Proyecto & Nombre, superficie de techo (m$^2$), capacidad del estanque (L), eficiencia de captación (\%) \\
...
Los resultados se organizan en tres pestañas principales:
...
\begin{enumerate}
    ...
    \item \textbf{Curva de optimización:} capacidad óptima recomendada y eficiencia alcanzable.
...
\midrule
\texttt{app\_scall.py} & Aplicación principal Streamlit: UI, visualizaciones Plotly, generación de PDF e informe Excel \\
...
\texttt{requests} & Consultas a APIs externas (altitud, coordenadas) \\
\bottomrule
\end{tabular}
\end{center}
